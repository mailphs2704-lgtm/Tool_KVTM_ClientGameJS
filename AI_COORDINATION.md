...
| 2026-09-16 | AUTO/Multi | AUTO/Multi | `a37bc35e..14adfb19` | Live sau fix trước vẫn kẹt `Chờ ClientJS vào game`: popup Sự kiện che `friend_off`, nên semantic gate sai dù client/Bridge đã sẵn sàng. AUTO PRO uploaded nay attach client đang mở, tuyệt đối không app_stop/app_start lúc startup; readiness=app_current+2 CAPTURE V3 frame liên tiếp trong 30s, sau đó bounded popup cleanup. Device catalog lấy cả ownership CRY+DEV, identity=`owner:profile_id`, resolve profile file theo owner và attach PID hiện hành. AST/static contract PASS; Windows build/live pending. |
| 2026-09-16 | AUTO/Multi | AUTO/Multi | `d4d80976..5e5ca175` | Live Cry AUTO PRO gốc: sau openGame restart, adapter dùng PID làm device id nên AUTO giữ PID cũ, Multi ownership không claim PID mới; đồng thời bytecode Android chờ `tai_khoan`/`icon_game` dù ClientJS vào thẳng game, gây treo `Tìm Icon Game`. Fix device id thành `CRYPROFILE:<profile_id>`, EngineDriver bind profile cố định; riêng owner CRY reclaim PID thay thế chỉ khi full profile signature khớp. Patch duy nhất startup boundary: restart ClientJS→Bridge→friend_off trong 90s→delay/cleanup; cấm emulator icons. AST PASS; Windows build/live pending. |
| 2026-09-16 | AUTO/Multi | AUTO/Multi | `d3b83a41..6cfeb1a6` | Theo operator chọn phương án 2 và đính chính không dùng AUTO_PRO cũ: Cry có nút `AUTO PRO gốc` mở process/GUI riêng từ `AUTO_PRO_ORIGINAL`. Package này dựng bằng allowlist runtime/pyc+_internal+assets của snapshot upload `autoproreal.zip` SHA256 `d3366f3e...`, offline API stateless; chỉ liệt kê ownership CRY, dùng EngineDriver/Bridge V3, fail-close ADB; cấm local_launcher/local_bridge/clientjs_auto_patch/Web/platform-tools. Không thay Auto Multi DEV. AST PASS; Windows build/live pending. |
