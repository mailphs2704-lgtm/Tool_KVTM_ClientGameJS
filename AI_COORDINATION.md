...
| 2026-09-19 | AUTO/Multi | AUTO/Multi | `PENDING` | Tinh gọn UI theo operator: ẩn tab Tự tạo AUTO/Xóa VP KC/Nâng kho/Giao cừu/Sản xuất ngọc/Dọn quầy/Thiết kế, đổi tab AUTO MULTI DEV thành Auto; ngắt scheduler + worker poll Dọn quầy tích hợp nhưng giữ module tương thích để tool Dọn quầy độc lập không bị ảnh hưởng. Function hiển thị 01/02/03, menu còn nửa chiều rộng và lưu/hiển thị theo acc (running config authoritative). Bỏ nút Dừng riêng, Bắt đầu thành toggle Bắt đầu/Dừng theo acc đang chọn. Static AUTO sale/warehouse/clear-stall PASS; Windows visual/live pending. |
| 2026-09-19 | AUTO/Multi | AUTO/Multi | `PENDING` | Đồng bộ cửa sổ Cấu hình Nâng kho với Cấu hình AUTO MULTI DEV: cùng header/body, Section/Auto LabelFrame, spacing, hàng nút Lưu/Mặc định/Đóng và helper đặt cửa sổ trong Multi. Chỉ đổi UI; persistence, mode, chu kỳ và consent dùng KC giữ nguyên. Static contract PASS; Windows visual pending. |
| 2026-09-19 | AUTO/Multi | AUTO/Multi | `PENDING` | Nâng kho thêm xác nhận per-profile dùng 1 KC xóa VP, mặc định OFF. OFF: chỉ quét ô trống View 1; quầy đầy thì đóng quầy, kết thúc lượt Nâng kho như safe skip và tiếp tục Function, không recovery/retry. ON mới gọi luồng xóa VP bằng KC. Static contract PASS; Windows live pending. |
| 2026-09-18 | AUTO/Multi | AUTO/Multi + Workspace | `db114c2` | Thêm cooperative level-up guard dùng `lv_up.png`: phát hiện toàn AUTO Main không tạo thread mới; click đúng Nhận `(500,688)`, prove popup đóng, unknown→exact MAIN, retry cùng Function loop không tăng counter. Startup sweep dùng cùng claim path. Checkpoint `docs/AUTO_LEVEL_UP_RECOVERY_CHECKPOINT_20260918.md`; source/AST PASS, Windows live pending. |
| 2026-09-17 | AUTO/Multi | AUTO/Multi + Workspace | `d1db1674` | Thêm tùy chọn per-profile `Sx cám` cạnh `Thăm bạn`. Chỉ chạy tại safe boundary sau Function PASS + bán VP, chu kỳ monotonic 35 phút. Chuỗi operator-confirmed: NPC sói → máy xay click x2/1.5s → panel proof → wheat BATCH_SWIPE → timer proof → X → Nhà → exact-main. SAFE_ABORT/recovery rồi retry sau sale kế tiếp; không UI poll/thread mới. Targeted static PASS; build/live pending. |
| 2026-09-17 | AUTO/Multi | AUTO/Multi + Workspace | `a61aaa7c` | Operator khoanh regression: bắt đầu sau fixed top-right, nặng hơn sau FPS. Ngoài loader đồng bộ/launch sát, top-right còn poll HWND 50ms/client trên Tk và FPS bị cap sớm + reassert 2s trong 60s tải ZingPlay. Sửa launch queue 4s, Bridge attach thread/PID, position one-shot sau display-ready, loading uncapped; FPS chỉ do menu explicit hoặc AUTO worker khi chạy. Launch/GPU/Bridge/Cry static PASS; Windows build/live pending. |
| 2026-09-17 | AUTO/Multi | AUTO/Multi + Workspace | `1cd3a29a..e8db2fb3` | Giảm tải log DEV: diagnostic sidecar default-off, detail log batch 0.75s, viewer tail 512 KiB/1000 action rows. Bridge V3 touch/capture dùng SendMessageTimeoutW 1s/2s để một ClientJS hung không khóa pipe vô hạn; contract CAPTURE3/INPUT4/WRITERMAP2/WRITERMSG1 giữ nguyên. Targeted static PASS; Windows build/live pending. NEXT Control Center [1] rồi [2], test >=3 client với 1 client treo ZingPlay. |
| 2026-09-15 | AUTO/Multi | AUTO/Multi | `0b5af84c..3953e185` | Stable publish từng kẹt 0.1.5 nên Cry chạy source 4e4a9f2 cũ, chưa nhận DWM-off/FPS hard-cap/pin vị trí/UI cấu hình hiện hành. Release nay tự tăng patch theo manifest khi HEAD mới (dự kiến 0.1.6), giữ cùng version nếu cùng HEAD; package gate bắt buộc DEV host + Bridge V3 + GPU/fixed-position/settings/UI. Setup và successful updater switch refresh profiles.json/settings.json/clear-stall-history từ DEV nhưng cấm copy running_clients/PID/runtime. Build/live pending; NEXT publish, đóng/mở Cry, xác minh auto bump + parity + GPU. |
| 2026-09-15 | AUTO/Multi | AUTO/Multi | `978d103e..e7b6133d` | Live sale log chứng minh kho chọn đúng Trà đá (`kho_tra_da=0.9937`) nhưng dialog primary `tra_da=0.7514 < 0.78` gây false-negative và CANCEL. Giữ nguyên threshold 0.78; thêm `kho_tra_da` làm fallback dialog, quét tối đa 3 fresh frame và bắt buộc 2 PASS ổn định trước Đặt bán. Không thay asset vì ảnh operator cung cấp khớp mẫu kho hiện hành. Static contract đã khóa fallback/bounded retry; build/live pending. NEXT Control Center [1], retest sale Function 3; cần log `selected-item STABLE READY` → x10 PASS → POST_SALE OWN_STALL READY. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `ac31388c` | Operator xác nhận live test `Function 3 - Step 1` PASS sau build verifier fix. Chốt bằng chứng: nút test chỉ chạy Step 1; 36 Táo (30 tầng 1-5 + 6 tầng 6), về MAIN bằng goDown(1)+nút XUỐNG, lên tầng 1, chuyển trang tìm Trà sấy, sản xuất 9/9 và Sửa máy; kết thúc giữ tầng 1. Function 3 đầy đủ vẫn chưa nối AUTO Main; chờ mô tả Step 2. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `9f61c22a..d78ef87b` | Build `[1]` tại `e8b9acb` dừng ở `verify_auto_builder_contract_core.py` vì verifier còn khóa mode cũ `floor-demo`. Đã cập nhật hợp đồng sang `choices=(main,function-3-step-1,builder)`, bắt buộc worker chỉ gọi `FunctionThreeWorkflow.run_step_1()`, có terminal event riêng và cấm floor-demo quay lại. Sale contract cũng khóa Function 3 bán Nước hoa hồng/Trà đá/Vải vàng cùng nút test hàng dưới. Chưa gọi build PASS; NEXT pull/build `[1]` lại và gửi output. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `fde255a0..216ae883` | Dựng `FunctionThreeWorkflow` + `RecipeBook(function_3)` dùng chung RecoveryManager; Step 1 chạy MAIN → tầng 1, thu/gieo 30 Táo, tầng 6 thu/gieo 6 Táo, `goDown(1)` + nút XUỐNG về MAIN, lên tầng 1, thu VP/tìm `tra_say` bằng chuyển trang phải từng lần, sản xuất 9/9 và Sửa máy, kết thúc tại tầng 1. Bổ sung sale policy/nhận diện Nước hoa hồng + Trà đá + Vải vàng cho recovery chung. Xóa floor-demo; nút hàng dưới chỉ gọi `run_step_1`, Function 3 đầy đủ chưa nối AUTO Main và fail-close trước input. GitHub/local static + Windows live đều pending. NEXT `[1]`, bấm Test Function 3 - Step 1 và gửi action/detail log. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `959401db` | Audit theo cảnh báo operator xác nhận planting retry sửa đúng runtime ownership: FunctionTwoWorkflow → RoseOilRecipe → `self.auto.planting` → instance `PlantingActions` do KVAutomation sở hữu; Hồng30/Hồng5/Tuyết28 có đúng 3 call shared method. Verifier khóa KVAutomation ownership, đúng 3 Recipe calls và cấm Recipe dựng PlantingActions song song. 4 file AST PASS; live retest vẫn pending. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `443f82d2`..`7c911875` | Live 15:04 Function 2 Hồng tầng 6: RIPE chỉ xuất hiện lần 6/6, code cũ harvest rồi `continue` ra khỏi loop nên không mở picker/gieo lại. Planting nay mỗi attempt click chậu đầu thật `selected_path[1]`; sau RIPE harvest mở picker và gieo ngay cùng attempt (kể cả 6/6). Thêm `KVTM_ERROR_LOG_COLLECTOR.bat` + PS1 nền read-only: theo dõi action/detail/JSONL/host logs, gom lỗi với 10 dòng trước+12 sau vào `%APPDATA%\KVTM Multi DEV\error-captures`; không điều khiển Multi/Auto/ClientJS. Planting/verifier AST PASS; PowerShell Windows live pending. NEXT `[1]`, retest Hồng 5 tầng 6 và chạy BAT; xác nhận log `sau thu hoạch đã click chậu trống đầu` → seed READY → PASS. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `9c5f541c`..`e5cbde1e` | Operator chốt OPENED toàn quy trình: chỉ đủ khi reward modal đã đóng + rương giữa xuất hiện lại; lưu timestamp trước cleanup để startup resume truyền đủ deadline +1200s sang scheduler. Popup kho đầy mới: chỉ nhận `full_kho`/tiêu đề `KHO QUÁ TẢI RỒI!`, bỏ qua body thay đổi; chờ 2s giữa X → Back → panel X; trả STORAGE_FULL/FAIL, không OPENED/20m và không friend reset. 5 file AST + source marker audit PASS; storage-full và startup handoff chưa Windows live. NEXT `[1]`, test popup kho đầy và startup-resume; log phải có FAIL KHO_QUA_TAI hoặc startup OPENED + scheduler chờ đủ 20 phút. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `4737cadb`..`90b2a608` | FINAL operator flow supersedes animation experiments: entry→slot0→MỞ NGAY; modal proved once; no CAPTURE 3s→tap chest `(486,603)`; no CAPTURE 3s→claim-once `(500,702)`; no CAPTURE 3s→prove center chest visible; X→MAIN→OPENED. Removed continuous open-prompt/claim-text/4s gates. Scheduler only OPENED sets +1200s; COOLDOWN/non-OPENED retries after next VP sale. AST + marker contract PASS; chưa Windows live. NEXT `[1]`, verify three wait=3.0 logs and status-specific next check. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `85129a59`..`3a31536a` | Live 12:16 mắc tại clean-session-enter-game: persisted Pirate open prompt bị generic modal backdrop/X xử lý trước khi chest workflow chạy. Fix public resume entry; GameSession resume trước/during startup; PopupActions ensure loop cho protected callback quyền ưu tiên trước generic dismiss, kể cả modal xuất hiện sau portal entry. Resume hoàn tất open+claim+quiet+panel close, prove own-farm rồi startup mới mark exact-main. Lifecycle contract + 5 file AST PASS; chưa live. NEXT `[1]`, log phải có startup phát hiện modal → resume PASS → fresh startup MAIN READY. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `96ca33f4`..`e9df835c` | Operator báo claim và panel check gần đồng thời; khả năng CAPTURE3 chen sát INPUT4 trong claim animation. Fix claim-once → quiet 4.0s tuyệt đối không frame/CAPTURE/check → mới check panel+rương giữa. Không retry claim. Verifier+AST PASS; chưa live. Log 12:05 chỉ là startup và cho thấy refresh định kỳ 3 vòng đang BẬT, chưa chứa chest status. NEXT `[1]`, gửi log chest có quiet=4.0s rồi center stable/status OPENED. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `866a0219`..`9b64a51e` | Operator chốt exact chest sequence: entry phân loại normal/persisted modal; normal chỉ slot0+MỞ NGAY; open modal giữ đúng trạng thái 4s rồi tap chest; reward phải đủ 4s + claim text stable mới claim-once `(500,715)`; chờ modal tự đóng+rương giữa stable; X panel và prove exact-main mới OPENED. Contract+AST PASS; chưa Windows live. NEXT `[1]`, log phải có open settle4s → claim animation>=4s → center stable → proof exact-main → status OPENED. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `b91175ce`..`8d9ecac7` | Live cho thấy claim/panel return còn race với animation: click có thể bị khóa và hậu kiểm 6s hết sớm. Fix gate vùng chữ `Chạm để nhận quà` ổn định 0.60s trước đúng 1 claim tap; reward timeout10s; panel+rương giữa phải ổn định 0.60s trong timeout12s rồi mới X; không retry/Back. Verifier+AST PASS, chưa Windows live. NEXT `[1]`, gửi log proof=claim-text-stable → claim-once → proof=center-chest-stable → status=OPENED. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `d4acf2dc`..`17c25ee9` | Live 11:08 chứng minh claim `(500,715)` nhận quà và đóng reward modal PASS, nhưng hậu kiểm so với panel trước MỞ NGAY làm SAFE_ABORT giả → friend reset → dừng AUTO/login-exact-main conflict. Fix giữ actual reward frame; PASS khi reward art biến mất + panel/rương giữa trở lại. Status OPENED nên tuyệt đối không qua nhà bạn. Verifier/AST PASS; chưa Windows rebuild/live PASS. NEXT `[1]`, retest và gửi log từ `claim-reward-once` đến `status=OPENED`. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `613e24b`..`0f2e463` | Operator cung cấp 2 ảnh reward flow và xác nhận quà ngẫu nhiên, click rương/vật phẩm không đóng modal. Workflow bỏ coin-only detector; dùng stable transition. Sau MỞ NGAY chứng minh vùng rương giữa biến mất; claim đúng 1 lần tại logical `(500,715)` ngay dưới chữ; cấm Back trên reward timeout; chỉ PASS khi panel+rương giữa xuất hiện lại rồi mới X về MAIN. Static contract khóa hành vi; workflow/verifier AST PASS. Chưa Windows build/live PASS. NEXT `[1]`, kiểm tra log claim-once + center-chest-returned + MAIN. |
| 2026-09-14 | AUTO/Multi | AUTO/Multi | `5f0bd974`..`ca9c91a5` | Operator xác nhận modal `Chạm để mở rương` có thể được game lưu khi rương đã chọn nhưng chưa mở; lần sau click thân thuyền vào thẳng modal. Entry nay nhận `panel OR open_prompt`; nếu open_prompt thì tiếp tục rương hiện tại, cấm replay slot/MỞ NGAY. Evidence 1000x1000 đạt header_mean15.12/orange_ratio0.397/open_prompt True. SAFE_ABORT friend reset chỉ còn chạy khi bước resume thật sự lỗi/timeout. AST/source contract PASS; chưa Windows build/live PASS. NEXT `[1]`, retest cả modal persisted và normal panel. |
| 2026-09-13 | AUTO/Multi | AUTO/Multi | `20417425`..`a8527e7a` | Live 23:22: Pirate Chest mở xong nhưng `reward-timeout`; HUD phía sau overlay tạo exact-main PASS giả, Function 2 chạy sai màn hình rồi Planting fail-close. Fix chỉ với `SAFE_ABORT`: back khỏi overlay một lần, bắt buộc dùng FriendRefresh route nhà bạn #1 → nhà mình, fail-close nếu vòng reset chưa PASS; OPENED/COOLDOWN/STORAGE_FULL không qua nhà bạn. Static source verifier + AST schedule PASS; chưa Windows build/live PASS. NEXT Control Center `[1]`, retest lỗi rương và xác nhận log scene reset trước Function kế tiếp. |
