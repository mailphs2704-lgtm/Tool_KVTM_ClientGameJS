...
function enforceFixedAdminPassword() {
  ...
  if (!verifyPassword(FIXED_ADMIN_PASSWORD, admin)) {
    const hp = hashPassword(FIXED_ADMIN_PASSWORD); admin.salt = hp.salt; admin.passwordHash = hp.hash; changed = true;
  }
  if (changed || !fs.existsSync(USERS_FILE)) {
    const tmp = USERS_FILE + '.tmp'; fs.writeFileSync(tmp, JSON.stringify(users, null, 2), 'utf8'); fs.renameSync(tmp, USERS_FILE);
  }
...
// Keep the proven v0.6 H.264 core untouched.
...
function bridgeCall(port,method,pathname,payload=null,timeout=7000){return new Promise((resolve,reject)=>{const token=readToken();if(!token)return reject(Error('Launcher bridge chưa chạy. Hãy mở lại RUN_LOCAL.bat v0.9.'));const body=payload==null?null:Buffer.from(JSON.stringify(payload));const r=http.request({host:'127.0.0.1',port,path:pathname,method,timeout,headers:{'X-Auto-Bridge-Token':token,...(body?{'Content-Type':'application/json','Content-Length':body.length}:{})}},x=>{const a=[];x.on('data',c=>a.push(c));x.on('end',()=>{const raw=Buffer.concat(a).toString('utf8');let j={};try{j=raw?JSON.parse(raw):{}}catch{return reject(Error(raw||`Bridge HTTP ${x.statusCode}`))}if((x.statusCode||500)>=400||j.ok===false)return reject(Error(j.error||`Bridge HTTP ${x.statusCode}`));resolve(j);});});r.on('timeout',()=>r.destroy(Error(`Launcher bridge ${port} timeout`)));r.on('error',reject);if(body)r.write(body);r.end();});}
...
async function handleOptions(req,res){
  ...
  try{
    ...
    if(req.method==='POST'){const raw=await collect(req,65536);let b={};try{b=raw.length?JSON.parse(raw):{}}catch{return json(res,400,{error:'JSON không hợp lệ'})}const r=await bridgeCall(OPTIONS_PORT,'POST','/options',{key:String(b.key||''),enabled:!!b.enabled});return json(res,200,r);}
...
async function handleSettings(req,res,u){
  ...
  try{
    if(req.method==='GET'){
      ...
      const r=await bridgeCall(SETTINGS_PORT,'GET',`/settings?key=${encodeURIComponent(key)}`,null,9000);
      return json(res,200,r);
    }
    if(req.method==='POST'){
      const raw=await collect(req,512*1024);let b={};try{b=raw.length?JSON.parse(raw):{}}catch{return json(res,400,{error:'JSON không hợp lệ'})}
      const r=await bridgeCall(SETTINGS_PORT,'POST','/settings',{key:String(b.key||''),fields:Array.isArray(b.fields)?b.fields:[],action:String(b.action||'update')},10000);
      return json(res,200,r);
    }
    return json(res,405,{error:'method not allowed'});
  }catch(e){return json(res,500,{error:e.message});}
...
async function handleExactStart(req,res){
  ...
  if(!(await coreAuth(req)))return json(res,401,{error:'unauthorized'});
  try{
    const raw=await collect(req,65536);let b={};try{b=raw.length?JSON.parse(raw):{}}catch{return json(res,400,{error:'JSON không hợp lệ'})}
    const deviceId=String(b.device_id||'').trim(); if(!deviceId)return json(res,400,{error:'device_id required'});
    ...
    const live=await bridgeCall(TASK_PORT,'GET','/functions',null,5000); const rows=Array.isArray(live.functions)?live.functions:[];
    let opt=null;
...
async function handlePasswordChange(req,res){
  try{const current=await coreJson(req,'/api/me');if(current&&current.username===FIXED_ADMIN_USERNAME)return json(res,409,{error:`Mật khẩu admin được cố định là ${FIXED_ADMIN_PASSWORD}`});}catch{}
...
const server=http.createServer((req,res)=>{const u=new URL(req.url,`http://${req.headers.host||'localhost'}`),p=u.pathname;if(p==='/api/options')return handleOptions(req,res);if(p==='/api/options/settings')return handleSettings(req,res,u);if(p==='/api/auto/start-v08')return handleExactStart(req,res);if(p==='/api/account/password'&&req.method==='POST')return handlePasswordChange(req,res);return proxy(req,res);});
...
setTimeout(()=>server.listen(PUBLIC_PORT,HOST,()=>console.log(`AUTO KVTM WEB v0.9: http://${HOST}:${PUBLIC_PORT} (core ${CORE_PORT})`)),350);
