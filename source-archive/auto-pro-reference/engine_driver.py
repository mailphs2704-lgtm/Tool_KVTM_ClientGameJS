...
class EngineDriver(PCDriver):
    ...
    def swipe_points(self, points, duration: float = 0.5) -> None:
        ...
        speed = load_swipe_speed()
        # The original Android path is paced by ADB/server round trips. The
        # in-process bridge has almost no latency, so replaying the same value
        # directly makes the basket cross crop cells too quickly. Preserve the
        # requested timing while enforcing a human-like per-segment dwell.
        total_duration = max(
            speed["minimum_ms"] / 1000.0,
            requested_duration * speed["duration_multiplier"],
            speed["segment_ms"] / 1000.0 * (len(path) - 1),
        )
