...
__all__ = ["IcedTeaProductionActions"]


class IcedTeaProductionActions(ProductionPanelActions):
    """Queue exactly nine iced teas from the paged floor-6 machine panel."""

    MACHINE_POINT = (262, 917)
    PRODUCT_TEMPLATE = "tra_da"
    ...
    def _open_and_find_product(self) -> tuple[int, tuple[int, int], tuple[int, int]]:
        """Collect VP, prove the production panel, then page until tra_da appears."""
        click_count = 0
        open_round = 0
        product = None
        ...
        while True:
            ...
            if product is not None:
                self.context.log(
                    "AUTO Trà đá • panel SX VERIFIED bằng product anchor ngay trang hiện tại • "
            ...
            if empty_ready:
                self.context.log(
                    "AUTO Trà đá • panel SX VERIFIED bằng empty-slot anchor • "
            ...
            self.context.log(
                "AUTO Trà đá • CHƯA chứng minh panel SX đang mở • "
        ...
        for drag_attempt in range(1, self.DRAG_ATTEMPTS + 1):
            ...
            for verify_round in range(1, self.VERIFY_RECHECKS + 1):
                ...
                if missing is not None:
                    ...
                    self._close_panel()
                    raise ScreenTimeout(
                        f"Thiếu nguyên liệu khi xếp Trà đá {ordinal}/9"
                    )
            ...
            if drag_attempt < self.DRAG_ATTEMPTS:
                self.context.log(
                    f"AUTO Trà đá • xếp {ordinal}/9 chưa thấy ô trống giảm • "
                    f"thử lại gesture {drag_attempt + 1}/{self.DRAG_ATTEMPTS}"
                )
        return last_empty

    def produce_9_iced_teas(
        self,
        *,
        ...
        for ordinal in range(1, self.REQUIRED_COUNT + 1):
            ...
            if current >= empty_after:
                self._close_panel()
                raise ScreenTimeout(
                    f"Kéo Trà đá {ordinal}/9 không làm giảm ô trống sau retry: "
                    f"trước={empty_after}, sau={current}"
                )
            empty_after = current
            self.context.log(
                f"AUTO Trà đá • đã xác minh xếp {ordinal}/9 • ô trống còn={current}"
            )

        if empty_before - empty_after != self.REQUIRED_COUNT:
            self._close_panel()
            raise ScreenTimeout("Hậu kiểm Trà đá không đạt đúng 9/9 ô")
        if close_after_success:
            self._close_panel()
        else:
            self.context.log("AUTO Trà đá • giữ panel mở để bàn giao sang Sửa máy")

        self.context.log("AUTO sản xuất Trà đá hoàn tất • đã xác minh đủ 9/9 ô")
