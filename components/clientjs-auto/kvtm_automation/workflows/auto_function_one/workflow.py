...
class FunctionOneWorkflow:
    ...
    def _require_main_transition(self, label: str) -> None:
        """Business-stage gate; recovery algorithms remain outside the Function."""
    ...
    def _recover_direct_juice_miss_to_floor_2(self) -> None:
        """Delegate a failed floor6→floor2 candidate to shared recovery."""
        self.context.stage("auto-function-1-direct-juice-floor-fallback")
        self.context.log(
            "AUTO Nước táo • direct goDown(4) chưa chứng minh được tầng 2 • "
            "gọi RecoveryManager: unknown → exact-main → tầng 2"
        )
        self.recovery.recover_unknown_to_floor(
            2,
            "Nước táo direct-floor fallback",
            reason="direct-juice-floor-anchor-miss",
            max_passes=self.DIRECT_JUICE_MAIN_RECOVERY_PASSES,
        )
        self.context.stage("auto-function-1-direct-juice-floor2-recovered")
        self.context.log(
            "AUTO Nước táo • fallback PASS • RecoveryManager đã đưa về candidate tầng 2 • "
            "production sẽ xác minh nuoc_tao lần nữa"
        )

    def _normalize_end_of_loop_to_main(self) -> None:
        """Delegate known floor-3 end-loop normalization to shared recovery."""
    ...
    def run(self) -> FunctionOneResult:
        ...
        juice = self.recovery.run_production(
            floor=2,
            label="Nước táo",
        ...
        self.recovery.to_main_from_floor(
            2,
            "sau SX Nước táo → trước trồng Bông",
        )
