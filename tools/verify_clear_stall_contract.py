...
def main() -> int:
    ...
    require(engine_driver, "class EngineTouchProxy:", "Bridge V3 cursor-free touch proxy missing")
    require(engine_driver, "self.touch = EngineTouchProxy(self)", "Resident touch proxy must be replaced")
    require(engine_driver, "def click(self, x: float, y: float)", "Bridge V3 click override missing")
    require(engine_driver, "return self.swipe_points(", "Bridge V3 swipe override missing")
    ...
    require(engine_driver, "verified_key != snapshot_key", "Bridge V3 seqlock post-copy verification missing")
    require(engine_driver, "frame_id < expected_frame", "Bridge V3 must accept stable newer completed frames")
    forbid(engine_driver, "status != 2 or frame_id != expected_frame", "Obsolete exact-frame Capture3 gate must stay removed")
    ...
    require(dev_entry, "def _close_clear_stall_client", "Dọn quầy terminal ClientJS cleanup helper missing")
    require(dev_entry, "def _confirm_clear_stall_client_off", "Dọn quầy OFF confirmation missing")
    require(dev_entry, '"incomplete-purchase-or-resale"', "Incomplete Dọn quầy transaction must close its ClientJS")
    require(dev_entry, '"probe-stopped"', "Manual Dọn quầy stop must close its ClientJS")
    require(dev_entry, '"probe-error"', "Dọn quầy failure must close its ClientJS")
    require(dev_entry, "current is not target_proc", "Dọn quầy cleanup must protect replacement PID")
    require(dev_entry, "self.processes.pop(profile_id, None)", "Confirmed Dọn quầy exit must publish OFF")
    require(auto_multi_worker, "while True:", "AUTO MULTI DEV restart-resume loop missing")
    require(auto_multi_worker, 'automation.driver.app_stop("")', "AUTO MULTI DEV exact-client stop missing")
    require(auto_multi_worker, 'automation.driver.app_start("")', "AUTO MULTI DEV same-profile relaunch missing")
    require(auto_multi_worker, '"client_pid_changed"', "AUTO MULTI DEV replacement PID event missing")
    require(auto_multi_worker, 'resume_config["skip_initial_sale_once"] = True', "AUTO MULTI DEV restart sale handoff missing")
    require(auto_multi_worker, "GameSessionWorkflow(automation).run", "AUTO MULTI DEV restart login/popup watch missing")
    require(auto_multi_worker, '"client_restart_completed"', "AUTO MULTI DEV restart completion event missing")
    require(dev_entry, "def _adopt_restarted_auto_client", "AUTO MULTI DEV GUI PID adoption missing")
    require(dev_entry, "current_pid not in {0, int(old_pid), int(new_pid)}", "AUTO MULTI DEV must reject stale replacement PID")
    require(dev_entry, "core.RunningProcessRef(int(new_pid))", "AUTO MULTI DEV GUI must bind exact replacement PID")
    ...
    print("terminal_client_state=confirmed-off-on-stop-error-incomplete")
    print("auto_multi_restart=same-profile-new-pid-login-watch-resume")
